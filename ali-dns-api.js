...
const ALI_ACCESS_KEY = process.env.ACCESS_KEY;
const ALI_ACCESS_SECRET = process.env.ACCESS_SECRET + '&';

exports.describeDomainRecords = function (domain, rrKeyWord, typeKeyWord, valueKeyWord) {
...
exports.describeSubDomainRecords = function (subDomain, type) {
    const queryString = 'Action=DescribeSubDomainRecords&SubDomain=' + subDomain + '&PageNumber=1&PageSize=20&Type=' + type;
    return request('GET', queryString);
}

exports.updateDomainRecord = function (recordId, rr, type, value) {
...
const request = function(httpMethod, queryString) {
    var date = new Date();
    const fulQueryString = queryString + '&Format=JSON&Version=2015-01-09&AccessKeyId=' + encodeURIComponent(ALI_ACCESS_KEY) + '&SignatureMethod=HMAC-SHA1'
    ...
    const signature = sign(strToSign, ALI_ACCESS_SECRET);
